...
public final class HttpSocket implements Connecter {
    ...
    public static Builder builder(boolean secure) {
        return new Builder() {
            ...
            public Connecter create(NinioProvider ninioProvider) {
                ...
                return new HttpSocket(ninioProvider.executor(), httpClient, path, connectAddress, secure);
            }
        };
    }
    ...
    private HttpSocket(final Executor executor, HttpConnecter httpClient, String path, final Address connectAddress, boolean secure) {
        this.executor = executor;

        HttpRequest request = new HttpRequest(new HttpRequestAddress(Address.ipToString(connectAddress.ip), connectAddress.port, secure),
                HttpMethod.POST,
                path,
